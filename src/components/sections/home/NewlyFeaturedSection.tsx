...
export function NewlyFeaturedSection({ restaurants }: { restaurants: FeaturedRestaurant[] }) {
        ...
        {restaurants.map(restaurant => {
          ...
          return (
            <Link
              ...
              className="relative shrink-0 w-[72vw] max-w-[280px] aspect-[5/4] rounded-2xl overflow-hidden block bg-gray-900"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                <p className="text-white text-[14px] font-bold leading-tight truncate">{restaurant.name}</p>
                ...
                {restaurant.cost_for_two && (
                  <p className="text-white/45 text-[10px] mt-0.5">₹{restaurant.cost_for_two} for two</p>
                )}
              </div>
            </Link>
          )
